 ...
 */
function generatePaymentSchedule(startDate, monthlyRent, numberOfPayments = 52, depositAmount = 0) {
  const schedule = [];
 ...
 */
function calculateProRataRent(monthlyRent, startDate, endDate) {
  const start = moment(startDate);
  const end = moment(endDate);
  const daysInPeriod = end.diff(start, 'days') + 1;
  const dailyRate = monthlyRent / 28;
 ...
 */
function calculateFinalPayment(lastPaymentDate, terminationDate, monthlyRent, currentBalance) {
  const proRataRent = calculateProRataRent(monthlyRent, lastPaymentDate, terminationDate);
 ...
 */
function calculateNoticeEndDate(noticeDate, noticePeriodDays = 28, lastPaymentDate) {
